...
Architecture
------------
Each NeuralNetwork (default NeuralNetworkImpl) is defined by a list ot Layrs. Each layer has a set of Connectins (default ConnectionsImpl) that link it to the other layers of the network. Each neural network is esentially a directed acyclic graph. This structure can accomodate simple feedforwad networks, but also more complex architectures like http://www.cs.toronto.edu/~hinton/absps/imagenet.pdf.
...
This tier is propagating data through the network. It takes advantage of the graph structure of the newtork. There are two main base components:
...
- AparapiConv2DSoftReLU - soft ReLU convolutional layer
