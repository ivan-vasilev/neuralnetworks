...
public class OpenCLSigmoid extends OpenCLTensorFunction
{
	...
	public OpenCLKernelReference createKernel()
	{
		...
		int id = OpenCLCore.getInstance().Sigmoid(deviceId, tensorRef.getId(), tensor.getSize(), tensor.getStartIndex());
	...
	{
		return " -D SGSI" + order + "=" + tensor.getStartIndex();
	}
}
