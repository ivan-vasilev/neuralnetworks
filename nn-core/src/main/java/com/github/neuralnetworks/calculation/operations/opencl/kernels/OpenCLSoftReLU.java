...
public class OpenCLSoftReLU extends OpenCLTensorFunction
{
	...
	public OpenCLKernelReference createKernel()
	{
		...
		int id = OpenCLCore.getInstance().SoftReLU(deviceId, tensorRef.getId(), tensor.getSize(), tensor.getStartIndex());
	...
	{
		return " -D SRSI" + order + "=" + tensor.getStartIndex();
	}
}
