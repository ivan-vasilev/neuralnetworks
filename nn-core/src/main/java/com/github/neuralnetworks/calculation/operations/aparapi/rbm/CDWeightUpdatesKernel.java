...
public class CDWeightUpdatesKernel extends Kernel implements Serializable
{
	...
	public CDWeightUpdatesKernel(Matrix posPhaseVisible, Matrix posPhaseHidden, Matrix negPhaseVisible, Matrix negPhaseHidden, Matrix weights, float learningRate, float momentum, float l1weightDecay,
			float l2weightDecay)
	{
		super();
		...
		this.miniBatchSize = posPhaseVisible.getRows();
	}
	...
	public void run()
	{
		...
		for (int i = 0; i < weightsColumns; i++)
		{
			...
			{
				weightUpdate += posPhaseHidden[posPhaseHiddenStartIndex + id * posPhaseHiddenColumnStep + j * posPhaseHiddenRowStep]
						* posPhaseVisible[posPhaseVisibleStartIndex + i * posPhaseVisibleColumnStep + j * posPhaseVisibleRowStep]
						- negPhaseHidden[negPhaseHiddenStartIndex + id * negPhaseHiddenColumnStep + j * negPhaseHiddenRowStep]
						* negPhaseVisible[negPhaseVisibleStartIndex + i * negPhaseVisibleColumnStep + j * negPhaseVisibleRowStep];
			}
