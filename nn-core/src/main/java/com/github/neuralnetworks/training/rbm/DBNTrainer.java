	...
	public void handleEvent(TrainingEvent event)
	{
		...
		if (event instanceof LayerTrainingFinished)
		{
			...
			if (list.indexOf(current) < list.size() - 1)
			{
				...
				if (current.getVisibleBiasConnections() != null && next.getVisibleBiasConnections() != null
						&& current.getVisibleBiasConnections().getWeights().getSize() == next.getVisibleBiasConnections().getWeights().getSize())
				{
					TensorFactory.copy(current.getVisibleBiasConnections().getWeights(), next.getVisibleBiasConnections().getWeights());
				}

				if (current.getHiddenBiasConnections() != null && next.getHiddenBiasConnections() != null
						&& current.getHiddenBiasConnections().getWeights().getSize() == next.getHiddenBiasConnections().getWeights().getSize())
				{
