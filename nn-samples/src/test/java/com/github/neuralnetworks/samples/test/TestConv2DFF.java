...
//		configurations.add(new KernelConfiguration[] {conf3});;
	...
	public TestConv2DFF(KernelConfiguration conf)
	{
	...
	public void test()
	{
		...
		if (kernelConfiguration.testCpu)
		{
			...
			ConnectionCalculator cpuConv = OperationsFactory.conv2D();
			...
			try
			{
				...
				ConnectionCalculatorConv cpucc = (ConnectionCalculatorConv) cpuConv;
				AparapiConv2DFF cc = (AparapiConv2DFF) cpucc.getInputFunctions().get(0);
		...
		if (kernelConfiguration.testAparapi)
		{
			...
			ConnectionCalculator aparapiConv = OperationsFactory.conv2D();
			...
			// prepare the kernel
			aparapiConv.calculate(connections, aparapiVP, connection.getOutputLayer());
